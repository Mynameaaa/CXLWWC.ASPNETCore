...
namespace WWC._240711.ASPNETCore.Extensions.Middleware.源码.UseDeveloperExceptionPage
{
    ...
    /// </summary>
    public class CustomIDeveloperPageExceptionFilterTwo : IDeveloperPageExceptionFilter
    {
        public CustomIDeveloperPageExceptionFilterTwo()
        {
            Console.WriteLine("异常处理中间件初始化 Two");
        }

        public async Task HandleExceptionAsync(ErrorContext errorContext, Func<ErrorContext, Task> next)
        {
            Console.WriteLine("异常处理中间件被调用 Two");
